//! LambdaMart per-stage training oracle (LOSS-04 / Plan 06.3-03 Wave B): train a
//! plain boosted RANKING model under `Loss::LambdaMart` (NDCG target, the upstream
//! default) over the shared frozen ranking corpus and gate per-tree splits,
//! per-tree leaf values, per-iteration staged approximants, and final predictions
//! against the committed upstream catboost 1.2.10 `ranking_corpus/LambdaMart`
//! fixture at <= 1e-5.
//!
//! LambdaMart is LISTWISE / QUERYWISE (RAW approx, `isExpApprox == false`): per
//! group it sorts docs by approx descending and accumulates the per-ordered-pair
//! lambda gradient toward NDCG (the Newton hessian is filled despite
//! `maxDerivativeOrder == 1`), routed through the grouped seam
//! (`compute_gradients_grouped`). It rides the POINTWISE Newton leaf estimator (NOT
//! the Cholesky pairwise path). Predictions are RAW (identity — the ranking score).
//!
//! `leaf_estimation_backtracking: No` upstream ⇒ the trivial (un-backtracked)
//! gradient walker, i.e. a single full Newton leaf step — the clean case for the
//! pointwise leaf parity. Pinned params mirror the fixture `model.json`: depth 2,
...
//! Cosine, leaf method Newton, sigma 1, norm true, top -1 (full group). The
//! upstream l2_leaf_reg default for LambdaMart is ~0 (1e-20); pinned here.
//!
//! Integration test (under `tests/`) so it can depend on `cb-oracle`. The fixture
//! is produced OFFLINE / frozen via `gen_ranking_fixtures.py --loss LambdaMart`
//! (RUN-ONCE/COMMIT, D-08).
#![allow(clippy::unwrap_used, clippy::expect_used, clippy::panic, clippy::indexing_slicing)]

use std::path::PathBuf;

use cb_backend::CpuBackend;
use cb_compute::{LambdaMartMetric, LeafMethod, Loss};
use cb_data::Pair;
...
fn base_params(loss: Loss) -> BoostParams {
    BoostParams {
        ...
        depth: 2,
        learning_rate: 0.3,
        // LambdaMart's upstream l2_leaf_reg default is ~0 (1e-20 in the fixture).
        l2_leaf_reg: 1e-20,
        random_strength: 0.0,
        boost_from_average: false,
        // LambdaMart rides the pointwise Newton leaf (model.json leaf_estimation_method).
...
#[test]
fn lambdamart_oracle_per_stage() {
    let scenario = "ranking_corpus/LambdaMart";
    ...
    .unwrap_or_else(|e| panic!("LambdaMart training failed: {e:?}"));

    compare_stage(Stage::Splits, &model_json.split_borders(), &model.split_borders())
        .unwrap_or_else(|e| panic!("LambdaMart: splits diverged: {e:?}"));
    compare_stage(Stage::LeafValues, &model_json.leaf_values(), &model.leaf_values())
        .unwrap_or_else(|e| panic!("LambdaMart: leaf values diverged: {e:?}"));
    ...
    compare_stage(Stage::StagedApprox, &expected_staged, &staged)
        .unwrap_or_else(|e| panic!("LambdaMart: staged approx diverged: {e:?}"));
    ...
    compare_stage(Stage::Predictions, &expected_predictions, &predictions)
        .unwrap_or_else(|e| panic!("LambdaMart: raw predictions diverged: {e:?}"));
}
