//! Wave-1 multiclass training oracle (LOSS-02 / Plan 06.2-03): train plain-boosted
//! multiclass models under `MultiClass` (softmax — the cross-dimension-coupled
//! symmetric Hessian Newton solve) and `MultiClassOneVsAll` (separable per-dimension
//! diagonal sigmoid Newton), and gate per-tree splits, per-tree leaf values, the
//! per-iteration staged N-dim approx, AND the softmax/sigmoid probability
//! predictions against the committed upstream catboost 1.2.10 fixtures at <= 1e-5.
//!
//! Two scenarios:
//!   - multiclass_softmax   : `Loss::MultiClass`. Exercises the coupled softmax der
//!     + packed symmetric Hessian and the per-leaf symmetric Newton solve
//!     (hessian.cpp:22-52). The `Stage::Splits` assertion is ALSO the isolation
//!     check for the Task-1b multi-dim single-shared-accumulator split score
//!     (Cosine couples num/den inside the sqrt).
//!   - multiclass_onevsall  : `Loss::MultiClassOneVsAll`. The separable per-dimension
//!     diagonal sigmoid Newton (reuses the scalar Logloss Newton leaf math per dim).
//!
//! Both pin `score_function=Cosine` (the catboost MultiClass default, NOT the
//! scalar fixtures' L2), `leaf_method=Newton`, `leaf_estimation_iterations=1`
//! (Pitfall 2), depth 2, 5 iterations, learning_rate 0.1, l2_leaf_reg 3.0,
//! bootstrap_type=No, random_strength=0, thread_count=1. The 3-class target is the
//! terciles of numeric_tiny.y (the same rule the generator pins); the trainer
//! remaps the labels to [0,3) internally.
//!
...
/// The 3-class target: digitize numeric_tiny.y into {0,1,2} at its 1/3 and 2/3
/// quantiles (the SAME rule the generator's `_multiclass_target` pins). Returned as
/// `f64` labels (the trainer remaps them to the contiguous [0,3) index internally).
fn load_multiclass_target() -> Vec<f64> {
    let y = load_f64_vec(&fixture("inputs/numeric_tiny/y.npy")).unwrap();
    // Quantile thresholds at 1/3 and 2/3 (linear interpolation, matching numpy's
    // default 'linear' method — the generator uses np.quantile defaults).
    let q13 = quantile_linear(&y, 1.0 / 3.0);
    let q23 = quantile_linear(&y, 2.0 / 3.0);
    y.iter()
        .map(|&v| {
            if v < q13 {
                0.0
...
/// interpolate between the floor and ceil ranks. `np.digitize(v, [q13,q23])`
/// returns 0 for `v < q13`, 1 for `q13 <= v < q23`, 2 for `v >= q23` (the bins are
/// right-open at the lower edge), which the class mapping above reproduces.
...
fn base_params(loss: Loss) -> BoostParams {
    BoostParams {
        ...
        // MultiClass default split-score function is Cosine (NOT the scalar L2).
...
fn train_scenario(loss: Loss, scenario: &str) -> (Model, Vec<f64>) {
    ...
    let target = load_multiclass_target();
...
/// `approx[d*n + i]`, the whole `iterations * dim * n` flat vector) into the
/// OBJECT-MAJOR `(iterations, n, dim)` layout the fixture staged.npy stores (A4).
...
fn check_multiclass(loss: Loss, kind: MultiClassKind, scenario: &str) {
    let (model, staged) = train_scenario(loss, scenario);
    let model_json = load_model_json(&fixture(&format!("{scenario}/model.json"))).unwrap();
    let dim = model.approx_dimension;
    assert_eq!(dim, 3, "{scenario}: expected 3 approx dimensions");
    let n = load_multiclass_target().len();

    // Stage 1: Splits (ALSO the Task-1b multi-dim split-score isolation check).
    ...
    // Stage 4: Predictions (softmax / per-dim sigmoid Probability, object-major).
    ...
    // Class-label round-trip: the stored map recovers the original [0,1,2] labels;
    // the argmax Class prediction is one of them per object.
    ...
    assert_eq!(classes.len(), n, "{scenario}: one class per object");
    for &c in &classes {
        assert!(
            (0.0..=2.0).contains(&c) && c.fract() == 0.0,
            "{scenario}: class {c} must be an original label in {{0,1,2}}"
        );
    }
}

#[test]
fn multiclass_softmax_per_stage_oracle() {
    check_multiclass(Loss::MultiClass, MultiClassKind::Softmax, "multiclass_softmax");
}

#[test]
fn multiclass_onevsall_per_stage_oracle() {
    check_multiclass(
