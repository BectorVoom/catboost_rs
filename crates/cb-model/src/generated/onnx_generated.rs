// Generated by `prost-build` + `protox` (pure-Rust protoc replacement) from the
// OFFICIAL ONNX project's `onnx.proto3` schema, PINNED to tag `v1.17.0`
// (commit b8baa8446686496da4cc8fda09f2b6fe65c2a02c),
// https://raw.githubusercontent.com/onnx/onnx/v1.17.0/onnx/onnx.proto3 .
//
// Generated ONCE, out-of-band (NOT part of `cargo build` — mirrors the
// `flatc --rust`-then-commit convention documented in `crate::lib` for
// `generated/model_generated.rs` etc.). NEVER hand-edited (D-01 analogue for
// ONNX/prost). `prost_build::Config::default()` (no extra `type_attribute`
// calls) — the `::prost::Message`/`::prost::Oneof` derive macros already
// generate a `Debug` impl for every message/oneof (verified: it is emitted by
// the proc-macro, not visible as source text, so do not be misled by grepping
// this file for `derive(Debug)`). To regenerate: vendor a (possibly newer)
// `onnx.proto3` release, rerun `protox::compile` + `prost_build::Config::
// compile_fds`, and replace this file wholesale, updating the pinned
// tag/commit above.
//
// The pinned schema *release* is independent of the *semantic* `ir_version`/
// opset values this port's exporter emits (`ir_version=3`,
// `ai.onnx.ml` opset `2`, matching upstream CatBoost's own ONNX exporter
// byte-for-byte) — the schema is designed append-only/back-compat, so a newer
// schema release still lets the exporter emit the older, upstream-matching
// values (EXPORT-01 research.md "Opset drift" pitfall).
