"""Generate the Buckets-CTR parity fixture (E16, SPEC-CTRT-06/SPEC-CTRT-12 / A1).
...
repo's Buckets-CTR training path is compared against at <=1e-5. CI does NOT run
...
For a Buckets CTR at binclf, upstream emits one candidate column PER
`target_border_idx` in `0..targetClassesCount` (`GetTargetBorderCount`,
`ctr_helper.h:34-42`; the `(ctrIdx, targetBorderIdx, priorIdx)` nesting of
`greedy_tensor_search.cpp:400-428`) — i.e. BOTH the class-0 and the class-1
numerator. An engine that hard-pins `target_border_idx = 0` scores a strictly
smaller candidate set and can neither choose nor persist a class-1 split. The
anti-false-pass guard below asserts the committed model genuinely carries BOTH
indices, so SPEC-CTRT-12 is testable at all.

# Pinned recipe

  - `catboost==1.2.10`, `numpy.random.RandomState(0)`.
  - 60 rows, TWO categorical columns (cardinality 6 and 5), ZERO float columns.
  ...
  - `simple_ctr=["Buckets:Prior=0.5"]`, `combinations_ctr=[]`,
    `max_ctr_complexity=1` — simple Buckets CTRs only.
  - The target is CAT-DRIVEN so upstream actually selects CTR splits at both
    border indices; a weak signal collapses onto one index and the fixture
    would be vacuous for SPEC-CTRT-12. The guard asserts that outcome away.
...
def main():
    ...
    # --- MANDATORY anti-false-pass guard (the strongest in the plan) ----------
    # SPEC-CTRT-12 is testable ONLY if the committed model carries splits at
    # BOTH target_border_idx 0 and 1. If this fires, widen the corpus / signal —
    # do NOT weaken the assertion.
    ctrs = model_json["features_info"].get("ctrs", [])
    buckets = [c for c in ctrs if c["ctr_type"] == "Buckets"]
    assert buckets, "no Buckets CTR descriptor in model.json — fixture is vacuous"
    ...
    config = {
        "scenario": SCENARIO,
        "requirement": "SPEC-CTRT-06/SPEC-CTRT-12",
        "seed": 0,
        "catboost_version": "1.2.10",
        "thread_count": 1,
        "n_rows": N_ROWS,
        "description": (
            "Buckets-CTR end-to-end isolation (E16 / A1). Categorical-ONLY feature matrix "
            "(two columns, cardinality 6 and 5, zero float columns) so upstream "
            "float-quantization nondeterminism is structurally excluded. "
            "one_hot_max_size=1 routes both columns to the CTR path. "
            "simple_ctr=Buckets with combinations_ctr=[] and max_ctr_complexity=1 "
            "isolates the simple Buckets type: per-class numerator selection via "
            "target_border_idx, with BOTH indices 0 and 1 present in the committed "
            "model (the anti-false-pass guard asserts it). NEVER regenerated in CI."
        ),
        "params": PARAMS,
    ...
    print(f"buckets target_border_idxs: {sorted(idxs)}")
