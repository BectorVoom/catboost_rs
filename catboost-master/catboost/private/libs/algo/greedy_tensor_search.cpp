...
// === CB_INSTRUMENT_LOG sink (06.3-17, env-gated, inert when unset) ===
// Per-level RNG call-count fences inside GreedyTensorSearch, to attribute the
// per-tree pairwise split-search draw count the Rust YetiRankTreeSeeder must
// reproduce for trees 2+. RUN-ONCE/COMMIT (D-08/D-11); strict no-op when
// CB_INSTRUMENT_LOG is unset.
static void CbGtsInstrumentLog(const TString& line) {
    const char* path = std::getenv("CB_INSTRUMENT_LOG");
    if (path == nullptr) { return; }
    static std::mutex cbGtsInstrMtx;
    std::lock_guard<std::mutex> g(cbGtsInstrMtx);
    ...
    for (const auto& candidatesContext : candidatesContexts) {
        for (const auto& subList : candidatesContext.CandidateList) {
            for (const auto& candidate : subList.Candidates) {
                // 06.3-17: per-candidate BestScore distribution + StDev fence (the
                // GetInstance draw is the per-candidate normal/Gumbel consuming the
                // persistent RNG). Logs the draw count this single GetInstance cost.
                const ui64 ccBeforeInst = ctx.LearnProgress->Rand.GetCallCount();
                double score = candidate.BestScore.GetInstance(ctx.LearnProgress->Rand);
                CbGtsInstrumentLog(TStringBuilder()
                    << "{\"event\":\"cand_score_rng\",\"dist\":"
                    << (candidate.BestScore.Distribution == ERandomScoreDistribution::Gumbel ? "\"Gumbel\"" : "\"Normal\"")
                    << ",\"stdev\":" << candidate.BestScore.StDev
                    << ",\"draws\":" << (ctx.LearnProgress->Rand.GetCallCount() - ccBeforeInst) << "}");
...
static TSplitTree GreedyTensorSearchOblivious(
    ...
    for (ui32 curDepth = 0; curDepth < ctx->Params.ObliviousTreeOptions->MaxDepth; ++curDepth) {
        ...
        // 06.3-17: per-level RNG fence — BEFORE CalcScores.
        const ui64 ccLevelStart = ctx->LearnProgress->Rand.GetCallCount();
        CalcScores(data, currentSplitTree, scoreStDev, &candidatesContexts, fold, ctx);
        // 06.3-17: per-level RNG fence — AFTER CalcScores (the CalcScores GenRand
        // randSeed + any pairwise-path child-RNG seeding).
        const ui64 ccAfterCalcScores = ctx->LearnProgress->Rand.GetCallCount();
        ...
        // 06.3-17: per-level RNG fence — AFTER SelectBestCandidate (the per-candidate
        // BestScore.GetInstance normal/Gumbel draws).
        const ui64 ccAfterSelectBest = ctx->LearnProgress->Rand.GetCallCount();
        CbGtsInstrumentLog(TStringBuilder()
            << "{\"event\":\"gts_level_rng\",\"depth\":" << curDepth
            << ",\"cc_start\":" << ccLevelStart
            << ",\"cc_after_calcscores\":" << ccAfterCalcScores
            << ",\"cc_after_selectbest\":" << ccAfterSelectBest
            << ",\"calcscores_draws\":" << (ccAfterCalcScores - ccLevelStart)
            << ",\"selectbest_draws\":" << (ccAfterSelectBest - ccAfterCalcScores) << "}");
        fold->DropEmptyCTRs();
