            ...
            switch (Config.NoiseType) {
                case EYetiRankNoiseType::Gumbel:
                    for (ui32 docId = 0; docId < QuerySize; ++docId) {
                        ...
                        /* cb_instr_yetirng */ if (std::getenv("CB_INSTRUMENT_LOG")) { CbInstrumentLog(std::string(R"J({"event":"yeti_gumbel","u":)J") + std::to_string(uniformValue) + "}"); }
...
void UpdatePairsForYetiRank(
    ...
    // 06.3-17: log the per-call recalc randomSeed + blockCount so the Rust
    // YetiRankTreeSeeder's per-tree deriv/learnfold/leafval seeds can be matched
    // call-for-call against the trainer. Env-gated (no-op when CB_INSTRUMENT_LOG
    // unset); RUN-ONCE/COMMIT (D-08/D-11).
    if (std::getenv("CB_INSTRUMENT_LOG")) {
        CbInstrumentLog(std::string(R"J({"event":"update_pairs","random_seed":)J")
            + std::to_string(randomSeed)
            + R"J(,"block_count":)J" + std::to_string((unsigned long long)blockCount)
            + R"J(,"query_begin":)J" + std::to_string(queryBegin)
            + R"J(,"query_end":)J" + std::to_string(queryEnd) + "}");
    }
    NPar::ParallelFor(
        *localExecutor,
        0,
        ...
        [&](int blockId) {
            ...
            for (int queryIndex = from; queryIndex < to; ++queryIndex) {
                ...
                const ui64 cbQuerySeed = rand.GenRand();
                if (std::getenv("CB_INSTRUMENT_LOG")) {
                    CbInstrumentLog(std::string(R"J({"event":"update_pairs_query","random_seed":)J")
                        + std::to_string(randomSeed)
                        + R"J(,"query_index":)J" + std::to_string(queryIndex)
                        + R"J(,"query_seed":)J" + std::to_string(cbQuerySeed) + "}");
                }
