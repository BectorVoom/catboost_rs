...
| **SPEC-CTRT-15** | `.cbm` DECODES mean tables, round-trip exact | **E18** (fixture), **E19** (decode + upstream `.cbm` ≤1e-5), **E20** (save→load→save byte identity) | all-zero-`mean` anti-vacuity guard; malformed-blob typed rejection; **the stride-ambiguity branch: try 8, then 12, and STOP AND REPORT if 12 matches — never silently pick one**; **`cbm_test.rs:731-746 decode_ctr_parts_mean_ctr_type_is_typed_error` is INVERTED into `decode_ctr_parts_mean_ctr_type_round_trips_sum_and_count` (asserting the reconstructed `(7.0f32, 3i64)`), never deleted — the DECODE mirror of E20's encode-side inversion; a retained sibling negative case still rejects a blob matching neither stride** |
...
| **SPEC-CTRT-18** | every existing CTR oracle stays green at defaults | **E05** primarily; **regression scope of E02, E04, E05, E06–E11, E15, E16, E22** | all 11 named oracles green, under a **PER-FILE diff gate in three categories** (PLAN.md §3.2; the per-file table is in E15's and E16's Completion evidence). **ZERO DIFF REQUIRED (7):** `plain_ctr_oracle_test`, `ordered_ctr_oracle_test`, `tensor_ctr_oracle_test`, `tensor_ctr_e2e_oracle_test`, `multi_permutation_e2e_oracle_test`, `multi_permutation_fold_oracle_test`, `fstr_ctr_oracle_test` — `git diff --stat` over them prints nothing. (F08's `#[non_exhaustive]` does NOT reach any of the seven: `fstr_ctr_oracle_test` and `ctr_data_roundtrip_test` contain no `cb_model::Model` struct literal — an earlier claim of one was a miscount of a `-> CbModel {` return type and is withdrawn.) **MECHANICAL EDITS ONLY, ZERO ASSERTION CHANGES (4):** `ctr_split_scoring_test` (E09 `:41`/`:68` + `:384`/`:394`, E11 `:542`/`:576`/`:645`, E16 `:99,:148,:191,:249,:305`, E22 all five call sites again, F08 the `cb_model::Model` literal at `:518`), `ctr_feature_materialize_test` (E09, E22 — ADDITIVE test fns + widened args), `ctr_data_roundtrip_test` (E11 — ADDITIVE test fns 2/4 + the compile-forced `build_final_ctr` arg at `:101,:138,:143,:163`), **`s_order_ctr_bins_oracle_test`** (E09 — `materialize_ctr_feature` args at `:70`, 7→9; E22 — the same site again, 9→10 with `extra_cat_columns`; **mechanical arity only, CHANGE NO ASSERTION**). **A diff touching an EXISTING assertion in ANY of the eleven is a STOP-AND-REPORT condition** |
...
| **Δ4** | width check uses the pool's **DECLARED** cat width | **F08** (stored `cat_feature_count`, **plus `#[non_exhaustive]` + a constructor/builder surface on `cb_model::Model`** — the locked decision, P4-4) → **F09** (set at fit) → **F10** (compare against it) — **in that order**; the previous `F09 → F08` edge was an inverted producer/consumer edge | positive `fit(pool) → predict(same pool)` test with a one-hot-routed trailing column returns `Ok` — the CRITICAL-3 counterexample; **plus F08's external-construction guard in an INTEGRATION target, proving `#[non_exhaustive]` left no `Model` shape unexpressible from another crate** |
...
|---|---|---|
| **R1** | Multi-prior expansion changes tie-breaks corpus-wide | E14's fixture lands **before** E15's code; E15 sits **after** the E05 firewall; all 11 single-prior oracles are E15's mandatory regression scope, under the **PER-FILE diff gate in three categories** (PLAN.md §3.2). **ZERO DIFF REQUIRED (7):** `plain_ctr_oracle_test`, `ordered_ctr_oracle_test`, `tensor_ctr_oracle_test`, `tensor_ctr_e2e_oracle_test`, `multi_permutation_e2e_oracle_test`, `multi_permutation_fold_oracle_test`, `fstr_ctr_oracle_test`. **MECHANICAL EDITS ONLY, ZERO ASSERTION CHANGES (4):** `ctr_split_scoring_test` (E09/E11/E16/E22/F08 — signature-driven argument/field edits only), `ctr_feature_materialize_test` (E09/E22 — ADDITIVE test fns + widened args), `ctr_data_roundtrip_test` (E11 — ADDITIVE test fns + the compile-forced `build_final_ctr` argument), **`s_order_ctr_bins_oracle_test`** (E09/E22 — the compile-forced `materialize_ctr_feature` arity at `:70`, 7→9→10; mechanical arity only). **A diff touching an EXISTING assertion in ANY of the eleven is a STOP-AND-REPORT condition**; weakening or deleting any assertion is **forbidden** |
...
| **R15** *(new)* | `F08`'s `#[non_exhaustive]` migration silently weakens an assertion in one of the ~20–25 external `Model` literals it rewrites | The migration is declared **MECHANICAL ONLY** in F08's Files and Refactor constraints: a rewritten site must yield a `Model` **field-for-field identical** to the literal it replaced, and **no assertion may be touched**. Two of the files are float-only oracles from the shipped one-hot plan (`cbm_oracle_test`, `json_oracle_test`) plus `cb-train`'s `ctr_split_scoring_test` (now listing **F08** as an owning task in the per-file diff gate). No ZERO-DIFF oracle is affected: `fstr_ctr_oracle_test` and `ctr_data_roundtrip_test` contain no `cb_model::Model` struct literal (an earlier claim of one was a miscount of a `-> CbModel {` return type) |
