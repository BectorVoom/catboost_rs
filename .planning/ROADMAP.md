...
- [ ] 05-18-PLAN.md — GAP (ORD-01 / SC-1, bar (c)) — RE-PLANNED (Spike 001, commit 3cee12a, INVALIDATED the prior 05-18): the spike is a mathematical impossibility proof that (i) cb-train's online-CTR math (materialize_ctr_feature / online_ctr_prefix_binclf / calc_ctr_online_bin) is ALREADY bit-exact to upstream — the prior plan's "fix #3" CTR-reindex targeted correct code and is DROPPED; and (ii) the committed (upstream_avg_perm, upstream_avg_ctr_bins) pair is INTERNALLY INCONSISTENT and cannot serve as an offline oracle. New multi-wave plan: Wave 1 RE-INSTRUMENTS the live trainer (05-17 recipe; disk NO-GO gate; offline-only, never CI) to dump an ATOMIC self-consistent per-iteration (effective_learn_permutation, learn_target_class, averaging_permutation, ui8 CTR bins) tuple, commits live_trainer_self_consistent.json (supersedes the inconsistent blocker fixture), and VERIFIES self-consistency offline (replay through the UNMODIFIED materialize_ctr_feature == captured bins) BEFORE porting. Wave 2 re-applies proven fix #1 (create_folds learning_folds-FULL-Fisher-Yates-passes → upstream_avg_perm [11,18,15,29,...] bit-exact) + fix #2 (per-iteration structure-fold cycling Folds[Rand.GenRand() % learning_folds], pc=4 [0,2,0,2,2]) + corrects the AveragingFold/structure-fold THREADING so the (UNMODIFIED) CTR materialization is fed the correct per-tree fold state — CTR math is OUT OF SCOPE (git diff --exit-code enforced); re-pins every blast-radius oracle to the NEW self-consistent upstream value (no #[ignore]/assert_ne/pinned-delta/loosened tolerance) + commits the pc=4 e2e oracle ≤1e-5. Wave 3 full-suite regression; pc=1 NOT regressed (hard invariant — pc=1 regression ⇒ fix WRONG ⇒ STOP). FALLBACK (first-class): if the self-consistent oracle cannot be captured (build/disk NO-GO/self-consistency gate fails), STOP and checkpoint — defer bar (c) with the spike proof, production UNTOUCHED, no weakened state. (6 tasks across 3 waves; autonomous=false — Wave-1 toolchain checkpoint.)
