...
- [ ] 06.5-08-PLAN.md — Investigation GATE: locate in upstream catboost 1.2.10 SOURCE the mechanism that produces the BM25 estimated-feature stored border scale (splits.npy ±1.24 / -0.550486 vs raw scores O(1e-3)) — value-transform vs border-algorithm vs multi-permutation ordered averaging — cross-checked by an instrumented CB_INSTRUMENT_LOG RUN-ONCE dump of the exact pre-quantization estimated-feature values; records a binding DECISION (PATH-A reproducible / PATH-B upstream-irreducible) with file:line + numeric evidence into BM25-NORMALIZATION-DECISION.md (D-07, escalate-don't-weaken; no silent re-deferral). `autonomous: true`
