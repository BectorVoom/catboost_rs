...
- [x] 07.5-06-PLAN.md — Pairwise split scorer (split_pairwise: per-leaf linear-system build from the 7.4 4-channel handle + Cholesky solve == calculate_pairwise_score) + pairwise scan/update + a ranking/PairLogit fixture grows a matching tree; CLOSES the GPU-01 kernel surface — **COMPLETE** (4b33b90 Task 1 scorer+self-oracle / 2ad46d7 Task 2 grow-loop, both GREEN on gfx1100): `scan_update_pairwise_kernel` (4-channel inclusive prefix REUSING `block_scan_kernel` verbatim — the deferred 7.4 `ScanPairwiseHistograms`, D-7.4-06) + `pairwise_make_derivatives_kernel` (device der-sum scatter == `MakePointwiseDerivatives`) + `select_best_split_kernel` (deterministic argmax, lowest-index tie-break == `SelectBestSplit`). `launch_pairwise_split_score` (+ `_into`) consumes the FROZEN 7.4 4-channel handle DEVICE-RESIDENT, then a BOUNDED host read-back of the assembled per-(feature,bucket) statistics runs the small per-leaf dense **Cholesky solve + score via the FROZEN `cb_compute::calculate_pairwise_score`** (RESEARCH Open Q3 DECISION: host-side over the bounded system — a `#[cube]` dense SPD solve would re-derive, not transcribe, the frozen `pairwise_cholesky_solve`; the bulk pairwise histogram + der-sum scatter stay device-resident, D-05). `grow_oblivious_tree_pairwise` (+ `_into`): depth-1 host-light pairwise grow (fill → der-sum scatter → split score → O(1) BestSplit → partition split/update, ONE client). **MVP = depth==1 / leaf_count==1 (root); depth>1 surfaces a typed `CbError::OutOfRange` naming the partition-aware (leaf_count>1) pairwise assembly forward dependency** (SAME class as the pointwise grow loop). Cross-oracles INLINE: `score_split::pairwise` (device der-sums == `compute_der_sums`; device pairwise score == `calculate_pairwise_score`; device argmax == CPU pairwise winner EXACTLY; 4-channel scan == host ordered cumulative; `>CUBE_DIM` → typed cross-cube-carry error) + `grow_loop::pairwise` (device tree STRUCTURE == inline CPU `greedy_tensor_search_oblivious_pairwise` EXACTLY — split + per-object leaf_of; leaf values REPORTED). NEVER imports cb-train (the Plan-A feature-unification landmine); the pairwise SCORER fns are read-only from cb-compute (already a dep). **rocm gfx1100: score_split::pairwise + grow_loop::pairwise 4/4 GREEN, score/scan/leaf bit-exact (0.000e0), STRUCTURE EXACT for n {8,37,200}; full 7.5 suite 72/72 GREEN (no regression).** cuda + wgpu build clean; SC-4 cb-compute cubecl==0; cb-core/cb-model/cb-compute/cb-train byte-unchanged. NO deviations. **GPU-01 kernel surface CLOSED** (all scorers L2/Cosine/Solar/LOO/Sat + pairwise + the full device-resident grow loop exist). Forward deps (typed-guarded): pairwise cross-cube scan carry (>CUBE_DIM) + partition-aware deep pairwise assembly. gsd-tools CLI ABSENT → STATE/ROADMAP updated MANUALLY.
