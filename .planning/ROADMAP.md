...
**Plans**: 16 plans in 15 waves (additive isolation ladder waves 1-6: one-hot → permutation → Plain CTR → Ordered CTR → Ordered boosting → tensor CTR; gap-closure waves 7-9: wave 7 fixed the multi-fold permutation oracle (05-07) + built the ordered split-scoring subsystem in tree.rs (05-08, the ORD-02 re-scope per STATE.md 2026-06-14); wave 8 wires ordered boosting into train() + locks the FULL multi-tree ordered e2e oracle (05-10); wave 9 wires tensor CTRs into train() (05-09) — per 05-VERIFICATION.md, the ORD-02 e2e bar is a full multi-tree hard gate, no #[ignore]) — gap-closure waves 14-15 (05-15/05-16) close the two re-verification BLOCKERS: WR-01 permutation_count>1 pre-averaging draw position (ORD-01) and the failing ordered_structure_differs_from_plain wiring test (ORD-02).
...
- [ ] 05-15-PLAN.md — GAP 2 (WR-01, ORD-01): move the pre-averaging GenRand draw in create_folds to fire immediately before the AVERAGING fold's shuffle (idx == learning_folds) for ALL permutation_count, not before the first learning shuffle (idx == 1) via the removed `first_real_shuffle` flag — correct only for permutation_count=1 today, wrong for the production default permutation_count=4. Upstream-pinned (learn_context.cpp:524/575-578 fold-creation order; fold.cpp:43-95 shuffle==false consumes zero draws; objects_grouping.cpp:191 / shuffle.h:24-32 one Fisher-Yates pass per shuffled fold). permutation_count=1 draw order byte-identical (existing averaging_fold_permutation + tensor_ctr_e2e + ordered_boost_e2e oracles stay green); NEW integer-exact multi_permutation_fold_oracle_test locks the permutation_count>=2 draw order (D-03 Stage::Permutation, NO #[ignore]). (2 tasks)
...
- [ ] 05-16-PLAN.md — GAP 1 (ORD-02): resolve the only failing test at HEAD, ordered_structure_differs_from_plain. After the 05-12 identity-Folds[0] fix, permutation_count=1 Ordered structure search runs on the identity learning fold so per-segment L2 scores collapse to Plain-identical on this randomness-free synthetic dataset — the test's divergence assumption is invalidated, NOT the implementation (the Ordered branch is alive: ordered_boost_e2e_oracle_test 2/2 ≤1e-5, greedy_tensor_search_oblivious_ordered wired at boosting.rs:1416). Re-key the sub-test to permutation_count>=2 where the learning fold is a genuine non-identity permutation (option b), OR retire it in place with an in-file rationale pointing to ordered_boost_e2e_oracle_test as the authoritative ORD-02 structural check (option c fallback) + a tracked note. Aliveness gates (ordered_training_grows_a_full_finite_model, plain_path_still_trains) preserved; TEST-ONLY change, no production source touched; cb-train lib 130/130 + all e2e oracles stay green. (1 task)
