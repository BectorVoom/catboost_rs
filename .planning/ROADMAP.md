...
**Wave 17** *(gap closure — final SC-1/ORD-01 BLOCKER, bar (c); blocks on nothing — transcribes the 05-17 committed live-trainer ground truth into production)*

- [ ] 05-18-PLAN.md — GAP (ORD-01 / SC-1, bar (c)): close the pc=4 production-default train→predict e2e ≤1e-5 by fixing THREE COUPLED production errors SIMULTANEOUSLY (fixing any in isolation regresses the green locks) — (1) create_folds averaging permutation corrected to learning_folds FULL Fisher-Yates passes (cc=29 pc=1 / cc=87 pc=4 → upstream [11,18,15,29,...] bit-exact, superseding the 05-17 cc==learning_folds rule per live_trainer_ctr_bins_blocker.json), (2) per-iteration structure-fold cycling Folds[Rand.GenRand() % learning_folds] (pc=4 [0,2,0,2,2] → [A,B,A,B,B] per-tree structure), (3) materialize_ctr_feature PERMUTATION-ORDER bucket reindex matching ComputeOnlineCTRs(AveragingFold) bit-exact ([7,7,7,11,3,7,2,11,...]) — then RE-PIN every blast-radius oracle (pc=1/pc=2/pc=4 partition locks, tensor_ctr_e2e, ordered_boost_e2e) to the CORRECT non-compensating upstream value (no #[ignore]/assert_ne/pinned-delta/loosened tolerance) + COMMIT the bar-(c) pc=4 e2e prediction oracle (multi_permutation_e2e_oracle_test.rs, doc-framing corrected). Reconciles D-08: CI reads only committed frozen fixtures; no live build in CI. (3 tasks, 1 wave — coupled fixes land together)
