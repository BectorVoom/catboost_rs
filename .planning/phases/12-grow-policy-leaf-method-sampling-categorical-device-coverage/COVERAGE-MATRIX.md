...
| SC-4 | Per-family BENCH-02 speed as it lands | ⏳ **PENDING-KAGGLE** (Task 2 — speed not yet timed) |
| SC-5 | Documented per-family GPU coverage matrix (this file) | ✅ done; **correctness cells filled from the 2026-07-04 CUDA run**, speed cells pending |

Authoritative **correctness** sign-off (GPUT-18/19/09/17/10) is **DISCHARGED on Kaggle CUDA
2026-07-04** — all six families + both e2e fits PASS ε=1e-4. The **BENCH-02 speed** measurement
is the only remaining Kaggle item before every family flips fully to `device-covered`.
