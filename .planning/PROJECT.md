...
**Standing debt (accepted at v1.1 close — DISCHARGED in v1.2 Phase 15):** at v1.1 close `GPUT-14` (the milestone-wide ε=1e-4 Kaggle CUDA correctness sign-off row) was `Pending` and the Phase-10 (depth-1) + Phase-11 (depth-6) BENCH-02 Kaggle speed rows were un-run (BENCH-03 stitched from Phase-12/13). **All three were discharged in v1.2 Phase 15** by the single-session Tesla P100 CUDA run (`bench/phase15_cuda_oracle/result.json`, `single_session: true`): the aggregate GPUT-14 correctness row ran ALL-PASS ε=1e-4 across all 13 device families (HARD-01), and the depth-1 + depth-6 BENCH-02 rows were measured in the same session (HARD-02, 12 rows 29.1×–40.8×). See `bench/BENCH-03-SIGNOFF.md`, MILESTONES.md → Known Gaps, and STATE.md → Deferred Items.
...
| Close v1.1 with GPUT-14 aggregate + Phase-10/11 BENCH-02 as accepted debt | Per-family ≤1e-4 evidence + committed P100 runs substantiate the claim; the missing rows are confirmatory, not load-bearing | Accepted debt at v1.1 (formal override in 14-VERIFICATION.md) — DISCHARGED in v1.2 Phase 15 (single-session P100 run) |
