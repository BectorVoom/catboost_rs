...
- **RESOLVED (fixtures) → RE-SCOPED (ORD-05 training pipeline) — 2026-06-14.** catboost==1.2.10 is now installed in `.venv`; the `tensor_ctr_e2e/` + `ordered_boost_e2e/` fixtures were generated in-env (commit 301f54c) after fixing the generator (permutation_count via low-level CatBoost(params) API) and the cb-oracle parser (OnlineCtr splits, model_json.rs). ORD-02 e2e is now GREEN (2/2). ORD-05 e2e (tensor_ctr_e2e_oracle_test) is 2/3: the FULL train→predict case fails because `cb_train::train()` has NO categorical-column parameter — `cat_cardinalities` is hardcoded `&[]` (boosting.rs:910). The categorical-CTR TRAINING pipeline (cat ingestion → per-fold online-CTR materialization → CTR-split scoring during tree growth → bake ctr_data into Model) is UNBUILT; the apply path (predict_raw_cat / ModelSplit::Ctr / ctr inference) IS complete. This is a plan-scale implementation, NOT a fixture/wiring tweak. **User decision (2026-06-14): re-scope via `/gsd-plan-phase 5 --gaps`.** The oracle test (and a new cat-aware train entry point) must drive it; do NOT weaken the gate.
