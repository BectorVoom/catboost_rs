...
- [x] **FEAT-07**: Online-HNSW KNN estimated-feature bit-exact parity — bit-for-bit port of `library/cpp/online_hnsw` (+ `NHnsw` routines + libc++ heap) in `cb-compute/src/hnsw.rs`, oracled on the per-object neighbor set index-for-index against the instrumented `knn_neighbors` dump (`hnsw_neighbor_oracle_test.rs`, incl. approximate-path prefixes), HNSW made the default calcer backend (exact retained behind `KnnCalcer::new_exact`), and the XOR text+embedding end-to-end StagedApprox + Predictions now match upstream ≤1e-5 in order (`text_embedding_end_to_end_oracle_test.rs`), closing the last open ≤10⁻⁵ CPU gap. (completed 2026-07-12)
