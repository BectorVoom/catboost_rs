...
| LOSS-04 | Phase 6.3 | In progress (06.3-01: grouped der seam landed. 06.3-02 (Wave A): **QueryRMSE + QuerySoftMax** der arms wired + trained end-to-end ≤1e-5. 06.3-03 (Wave B): **LambdaMart** (NDCG listwise lambda grad + pointwise Newton leaf) trained end-to-end per-stage oracle ≤1e-5; **PairLogit/PairLogitPairwise** der + the Cholesky pairwise-leaf path (pairwise_leaves.rs, bit-exact vs upstream UT) + is_pairwise_scoring routing LAND + unit-tested, with the two PairLogit/PairLogitPairwise per-stage ORACLES DEFERRED on an isolated pair-weight normalization gap (build_query_info group-relative pair weighting follow-up; deferred-items.md). Remaining: YetiRank/StochasticRank (Wave C, Plan 04, instrumented) + the PairLogit oracle follow-up). |
